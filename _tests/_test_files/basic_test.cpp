...
bool bplustree_test()
{
  ...
  cout << "\n\n--- other functions --------\n\n";
  cout << "map.size():        " << bt.size() << endl;
  cout << "map.empty():       " << bt.empty() << endl;
  cout << "map.find(43):      " << *bt.find(43) << endl;
  cout << "map.contains(45):  " << boolalpha << bt.contains(45) << endl;
  cout << "map.contains(100): " << boolalpha << bt.contains(100) << endl;
  cout << "map.contains(150): " << boolalpha << bt.contains(150) << endl;
  cout << "map.is_valid():    " << bt.is_valid() << endl;
...
bool interface_test()
{
  ...
  for(int i=0;i<10;i++)
  {
    values[i] = 1+rand()%10000;
    bt.insert(values[i]);
  }
  cout<<bt<<endl;
  for(int i=9;i>=0;i--)
  {
    cout<<"Removing "<<values[i]<<endl;
    bt.remove(values[i]);
    cout<<"removed "<<values[i]<<endl;
    cout<<bt<<endl
    <<"-----------------------------------------\n";
  }
  // SQL sql;
...
// TEST(Multimap_Test, MultiMapTest) {
//   bool success = multimap_test();
//   EXPECT_EQ(success, true);
// }

TEST(Interface_Test, InterfaceTest) {
  bool success = interface_test();
  EXPECT_EQ(success, true);
}
